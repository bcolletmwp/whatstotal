...
import IntegrationModal from "../../components/TypeBotModal";
...
const QueueIntegration = () => {
  ...
  const handleDeleteIntegration = async (integrationId) => {
    try {
      ...
      toast.success(i18n.t("typeBot.toasts.deleted"));
    } catch (err) {
      toastError(err);
    }
  ...
  return (
    <MainContainer>
      <ConfirmationModal
        title={
          deletingUser &&
          `${i18n.t("typeBot.confirmationModal.deleteTitle")} ${deletingUser.name
          }?`
        }
      ...
      >
        {i18n.t("typeBot.confirmationModal.deleteMessage")}
      </ConfirmationModal>
      <IntegrationModal
        open={userModalOpen}
      ...
      />
      <MainHeader>
        <Title>{i18n.t("typeBot.title")} ({queueIntegration.length})</Title>
        <MainHeaderButtonsWrapper>
          <TextField
            placeholder={i18n.t("typeBot.searchPlaceholder")}
            type="search"
            value={searchParam}
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center">{i18n.t("typeBot.table.id")}</TableCell>
              <TableCell align="center">{i18n.t("typeBot.table.name")}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            <>
